...
const W = 1280, H = 720, FPS = 30, DUR = 10, PORT = 9513;
const OUT = path.join(__dirname, '..', 'brand', 'wage-backers-10s.mp4');
const FRAMES = path.join(__dirname, 'backers-frames');
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
(async () => {
  fs.rmSync(FRAMES, { recursive: true, force: true }); fs.mkdirSync(FRAMES);
  const url = 'file:///' + path.join(__dirname, 'backers-video.html').split(path.sep).join('/');
