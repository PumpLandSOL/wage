...
const W = 1280, H = 720, FPS = 30, DUR = 10, PORT = 9509;
const OUT = path.join(__dirname, '..', 'brand', 'wage-demo-10s.mp4');
const FRAMES = path.join(__dirname, 'wage-frames');
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
(async () => {
  fs.rmSync(FRAMES, { recursive: true, force: true }); fs.mkdirSync(FRAMES);
  const url = 'file:///' + path.join(__dirname, 'demo-video.html').split(path.sep).join('/');
